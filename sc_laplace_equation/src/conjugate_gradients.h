...
void conjugate_gradients(const utils::SparseMatrix& A, const utils::Vector& b, utils::Vector& x,
                         Scalar _error, int _iters)
{
    assert(A.rows() == A.cols());

    /**
     * \todo Implement the conjugate gradients solver as presented in the lecture.
     *
     * Besides the function parameters `x` and `b`, you will have to store only
     * three more vectors: residual `r`, direction `p`, and product `Ap = A * p`.
     *
