...
HeatEquationViewer::HeatEquationViewer(const char* _title, int _width,
                                       int _height)
...
HeatEquationViewer::~HeatEquationViewer()
{
  ...
  if (glIsTexture(textureID_)) glDeleteTextures(1, &textureID_);
}
...
void HeatEquationViewer::generate_grid(int _resolution)
{
  ...
  std::cout << "Construct " << grid_resolution_ << "x" << grid_resolution_
            << " grid..." << std::flush;
  ...
  for (int i = 0; i < grid_resolution_; ++i)
    for (int j = 0; j < grid_resolution_; ++j) V(i, j) = A(i, j) = 0.0;
}
...
void HeatEquationViewer::processImGUI()
{
  if (ImGui::CollapsingHeader("Heat Equation", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    if (animate_) ImGui::Text("Integration Time: %.3f ms", integration_time_);
  }
}
    ...
    case GLFW_KEY_1:
    {
      for (int i = 0; i < grid_resolution_; ++i)
        for (int j = 0; j < grid_resolution_; ++j)
          U(i, j) =
              0.20 *
              pow(sin(3.0 * M_PI * X(i, j)) * sin(3.0 * M_PI * Y(i, j)), 2.0);
    ...
    case GLFW_KEY_2:
    {
      for (int i = 0; i < grid_resolution_; ++i)
        for (int j = 0; j < grid_resolution_; ++j)
          U(i, j) =
              0.1 * X(i, j) + 0.1 * Y(i, j) +
              0.2 * pow(sin(4.0 * M_PI * X(i, j)) * sin(4.0 * M_PI * Y(i, j)),
                        3.0);
...
void HeatEquationViewer::pick_point()
{
  ...
  double p0[3], p1[3];
  gluUnProject(x, y_new, 0.0, modelview, projection, viewport, &p0[0], &p0[1],
               &p0[2]);
  gluUnProject(x, y_new, 1.0, modelview, projection, viewport, &p1[0], &p1[1],
               &p1[2]);

  // intersect ray with z=0 plane
  float l = -p0[2] / (p1[2] - p0[2]);
  vec3 p(p0[0] + l * (p1[0] - p0[0]), p0[1] + l * (p1[1] - p0[1]),
         p0[2] + l * (p1[2] - p0[2]));
...
void HeatEquationViewer::display()
{
  ...
  glDrawElements(GL_QUADS, grid_indices_.size(), GL_UNSIGNED_INT,
                 &grid_indices_[0]);
...
void HeatEquationViewer::explicit_euler_step()
{
   ...
   *   from `0` to `grid_resolution - 1`. Points at the boundary should stay
   * fixed.
  ...
  for (i = 1; i < grid_resolution_ - 1; i++)
  {
    for (j = 1; j < grid_resolution_ - 1; j++)
    {
...
void HeatEquationViewer::solve_equilibrium()
{
  ...
  /**
   * Update the height values `U(i, j)` at every grid point `(i, j)` by
   ...
   * - Take care when mapping a grid index `(i, j)` to an index `k` in the
   * vector `x` and the matrix `A`. Use the mapping `k = idx(i, j)` discussed in
   * the lecture.
...
#define IDX(i, j) ((i)-1) * (r - 2) + ((j)-1)
  ...
  // Define dimension (n x n) of matrix A
  int m = (r - 2) * (r - 2);

  // Declare the vectors x and b, initialize b with 0
  VectorX x(m), b(m);
  b.setZero();

  // Declare matrix A by its dimensions and initialize it with 0
  MatrixXX A(m, m);
  A = MatrixXX::Zero(m, m);
  ...
  // Use Eigen Cholesky solver to solve "A * x = b"
  // x = A.llt().solve(b);
  Eigen::LLT<MatrixXX> solver;
  solver.compute(A);
  if (solver.info() == Eigen::Success) x = solver.solve(b);
